//
//  SiPixelTemplateReco.cc (Version 10.10)
//
...

{
   ...
   double chi2x, meanx, chi2y, meany, chi2ymin, chi2xmin;
...
//   const double mean1pix={0.100}, chi21min={0.160};
   ...
   if(logypx == 1) {
      ...
      if(nypix ==1) {
         delta = templ.dyone();
         sigma = templ.syone();
         proby = templ.fracyone();
      } else {
         delta = templ.dytwo();
         sigma = templ.sytwo();
         proby = templ.fracytwo();
      }
...
//      proby = 1. - TMath::Gamma(hndof, hchi2);
   ...
   if(logxpx == 1) {
      ...
      if(nxpix ==1) {
         delta = templ.dxone();
         sigma = templ.sxone();
         probx = templ.fracxone();
      } else {
         delta = templ.dxtwo();
         sigma = templ.sxtwo();
         probx = templ.fracxtwo();
      }
